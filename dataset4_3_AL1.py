...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    if plot:
        ...
        plt.legend()
        plt.savefig("dataset4_3_AL1_Accuracy_Plot.png")
...
def test(data):
    ...
    total = 0
    with torch.no_grad():
        with open('dataset4_4_'+data+'.csv', mode='w') as test_file:
...
if __name__ == "__main__":
    ...
    data_dir = "/home/ba4_project/ba4_Hee/Trypanosome/training_dataset4_3_AL1"
    ...
    if training_only:
        ...
        torch.save(model_ft, "dataset4_3_AL1_best_model.pth")
    
    if prediction:
        model_ft = torch.load('dataset4_3_AL1_best_model.pth')
        model_ft.eval()
        print("\nPrediction Started")
        filename_list = image_datasets['unlabeled'].imgs

        with torch.no_grad():    
            with open('Prediction4_3_AL1.csv', mode='w') as prediction_file:
