...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    if plot:
        ...
        plt.legend()
        plt.savefig("dataset4_Accuracy_Plot.png")
...
def test(data):
    ...
    total = 0
    with torch.no_grad():
        with open('dataset4_0_'+data+'.csv', mode='w') as test_file:
...
if __name__ == "__main__":
    ...
    data_transforms = {
        ...
        'unlabeled_testvid_excl': transforms.Compose([
            transforms.Resize([224, 224]),
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
        ]),
    ...
    data_dir = "/home/ba4_project/ba4_Hee/Trypanosome/training_dataset4"
    test_data_dir = "/home/ba4_project/ba4_Hee/timothy_workstation/data/training_dataset1"
    image_datasets = {x: datasets.ImageFolder(os.path.join(data_dir, x),
                                              data_transforms[x])
                      for x in ['train', 'val', 'unlabeled_testvid_excl']}
    ...
    unlabeled_dataloader = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=batch_size,
                            ...
                            for x in ['unlabeled_testvid_excl']}
    nonshuffle_dataloaders = {x: torch.utils.data.DataLoader(test_datasets[x], batch_size=batch_size,
                                                            shuffle=False, num_workers=4)
                            for x in ['test1', 'test2', 'positive_test', 'test_vid01', 'test_vid02', 'test_vid03',
                                'test_vid05', 'test_vid06', 'test_vid07']}
    dataset_sizes = {x: len(image_datasets[x]) for x in ['train', 'val', 'unlabeled_testvid_excl']}
    ...
    print(f'Unlabeled image size: {dataset_sizes["unlabeled_testvid_excl"]}')
    ...
    if training_only:
        ...
        torch.save(model_ft, "dataset4_best_model.pth")
    
    if prediction:
        model_ft = torch.load('dataset4_best_model.pth')
        model_ft.eval()
        print("\nPrediction Started")
        filename_list = image_datasets['unlabeled_testvid_excl'].imgs

        with torch.no_grad():    
            with open('Prediction4_2.csv', mode='w') as prediction_file:
                writer = csv.writer(prediction_file, delimiter=',')
                writer.writerow(["Image", "Class", "PredictedProb"])

                for i, (inputs, labels) in enumerate(unlabeled_dataloader['unlabeled_testvid_excl']):
                    inputs = inputs.to(device)
                    outputs = model_ft(inputs)
                    m = nn.Softmax(dim=1)
                    prob = m(outputs.data)
                ...
                # print('image numbers = ', len(img_num_list), img_num_list)
                # print('neg_list=', len(neg_list), neg_list)
